...
Validation: rootless Podman is validated on Oracle Linux 9.8 (Podman 5.8.2). Other RHEL 9 family distributions ship the same Podman 5 and are accepted, not validated. Debian and Ubuntu are not validated; `get.sh` prints a warning there.
...
Keys are upper-case identifiers. Every value is written single-quoted, one per line (`KEY='value'`). `.env` is read three ways: `install.sh` sources it with bash, compose interpolates it, and the bundle's env reader parses it back. A single-quoted value is identical for all three when it contains `$`, whitespace, a backtick or ` #`; an unquoted `pa$w0rd` would source as `pa`. **A single quote (`'`) in a value is refused by the preflight**: there is no encoding of it that all three readers accept (bash's `'\''` makes Docker Compose reject the whole `.env`, after the license key is consumed). Rotate the credential, or percent-encode the quote as `%27` inside `DATABASE_URL` / `REDIS_URL`. Three rules on the YAML side:
...
CI runs the same commands on every pull request, with pinned tool versions, on ansible-core 2.15 (the declared floor) and on the current release. The `ansible.cfg` at the repository root only enables SSH pipelining (see "Upgrades"); it replaces, not extends, a `~/.ansible.cfg` when you run from the repository root.
